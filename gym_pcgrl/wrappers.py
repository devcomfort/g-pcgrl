...
class ToImage(Wrapper):
    def __init__(self, game, names, **kwargs):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=max_value, shape=(self.shape[0], self.shape[1], depth), dtype=np.uint8)
...
class SwapCropped(Cropped):
    def __init__(self, game, pad_value, name, **kwargs):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=high_value, shape=(kwargs["width"], 3, high_value), dtype=np.uint8) #5
        #for (k,s) in self.env.observation_space.spaces.items():
        #    self.observation_space.spaces[k] = s
